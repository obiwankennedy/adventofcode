...
variables={}
maxV=0
for instruction in inputVal.splitlines():
    tab=instruction.split(' ')
    valueA = tab[4]
    valueB = int(tab[6])
    valueA=int(variables[valueA]) if valueA in variables else 0
    value=int(variables[tab[0]]) if tab[0] in variables else 0
    op = tab[5]
    do=False
    if(op=='<' and valueA<valueB):
        do=True
    elif(op == '>' and valueA>valueB):
        do=True
    elif(op == "!=" and valueA!=valueB):
        do=True
    elif(op == ">=" and valueA>=valueB):
    ...
    if(do):
        if(tab[1]=="inc"):
            ...
            value-=int(tab[2])
        variables[tab[0]]=value
        if(value > maxV):
            maxV=value
count = str(max(variables.values()))
print("Solution: "+str(count))
print("Exercice 2")
print("Solution: "+str(maxV))
